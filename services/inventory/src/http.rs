...
use crate::repository::{self, StockRow};
...
impl From<StockRow> for StockRepresentation {
...
/// `PUT /_dev/stock/{sku}`: dev/test-only stock seeding (spec section 6).
async fn seed_stock(
    State(pool): State<PgPool>,
    Path(sku): Path<String>,
    Json(body): Json<SeedStockRequest>,
) -> Result<impl IntoResponse, ApiError> {
    let row = repository::seed_stock(&pool, &sku, body.available_qty, Utc::now()).await?;
    Ok(Json(StockRepresentation::from(row)))
}

/// `GET /_dev/stock/{sku}`: dev/test-only stock read.
async fn get_stock(
    State(pool): State<PgPool>,
    Path(sku): Path<String>,
) -> Result<impl IntoResponse, ApiError> {
    let row = repository::get_stock(&pool, &sku)
        .await?
        .ok_or_else(|| ApiError::StockNotFound(sku.clone()))?;
    Ok(Json(StockRepresentation::from(row)))
}
...
pub fn router(state: AppState) -> Router {
    let router = Router::new()
        ...
        .route("/_dev/stock/{sku}", put(seed_stock).get(get_stock));
