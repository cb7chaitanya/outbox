...
pub struct Config {
    ...
    /// Transient-failure retry budget (spec section 15 defaults):
    /// bounded attempts and elapsed time, full-jitter backoff between them.
...
impl Config {
    ...
    pub fn retry_config(&self) -> crate::consumer::RetryConfig {
        crate::consumer::RetryConfig {
            max_attempts: self.retry_max_attempts,
            max_elapsed: std::time::Duration::from_secs(self.retry_max_elapsed_secs),
            backoff_base: std::time::Duration::from_millis(self.retry_backoff_base_ms),
            backoff_cap: std::time::Duration::from_millis(self.retry_backoff_cap_ms),
        }
    }
}
...
/// only ever opens its own `fulfilment` database.
...
pub struct DatabaseConfig {
    ...
    pub postgres_fulfilment_db: String,
}
